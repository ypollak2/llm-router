...
An audit specialist named a **different, private project** of the operator's
when documenting the `127.0.0.1:17900` port-collision finding. This repository
is public. The name has been redacted; the finding is unchanged, because the
finding is about an unnamespaced, unauthenticated localhost port being
cross-reported as this product's own daemon — which does not require naming
the other process.

Caught by `scripts/check_identity.py` before the commit reached the remote.
That guard exists for exactly this and it worked.
