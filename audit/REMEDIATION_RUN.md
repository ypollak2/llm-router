...
**^ CONTAMINATED — see Phase 50 (`24_CLEAN_ROOM_REPRODUCTION.md`, P50-N1).**
`doctor --audit` increments `fail_open_events` by 4 per invocation, because two
idempotent `ADD COLUMN is_simulated` migrations fail with `duplicate column
name` on every database open and are recorded as fail-open events. An unknown
share of the 713 was generated by me running `doctor` during development, and
the share is unrecoverable. `CHZ-FO-COST-MIGRATE-ALTER` is not a degradation at
all — it is a migration correctly doing nothing. Remediation plan II, S1/S2.
