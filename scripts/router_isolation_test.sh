#!/bin/bash
# Test llm_router routing in isolated environments to verify:
...
main() {
    ...
    log "Starting llm_router isolation tests..."
    log "Project root: $PROJECT_ROOT"
    log "Log file: $LOG_FILE"

    # Verify dependencies
    if ! command -v llm_router &> /dev/null; then
        log "ERROR: llm_router not found in PATH"
        alert_failure "Router isolation test failed" "llm_router CLI not found"
        exit 1
    fi
    ...
    log "llm_router version: $(llm_router --version 2>/dev/null || echo 'unknown')"
...
# Run every 6 hours
0 */6 * * * /Users/yali.pollak/Projects/llm_router/scripts/router_isolation_test.sh >> /tmp/router_test.log 2>&1

# Run daily at 2 AM
0 2 * * * /Users/yali.pollak/Projects/llm_router/scripts/router_isolation_test.sh

# Run every hour (aggressive)
0 * * * * /Users/yali.pollak/Projects/llm_router/scripts/router_isolation_test.sh
