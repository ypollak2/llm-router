#!/usr/bin/env python3
"""Generate an asciinema v2 .cast file for the llm_router demo SVG."""
...
DEMO_LINES = [
    "",
    f"{B}llm_router demo{R}  — how smart routing handles your prompts",
    "",
    ...
    "",
    f"  {Y}→{R} Run {B}llm_router status{R} for cumulative savings.",
    f"  {Y}→{R} Run {B}llm_router dashboard{R} to see live routing decisions.",
    "",
]
...
STATUS_LINES = [
    ...
    "",
    f"  {B}Subcommands{R}",
    f"    {B}llm_router update{R}     — update hooks to latest version",
    f"    {B}llm_router doctor{R}     — full health check",
    f"    {B}llm_router dashboard{R}  — web dashboard (localhost:7337)",
    SEP,
    "",
]
...
def build_cast(out_path: Path) -> None:
    ...
    header = {
        ...
        "width": WIDTH,
        "height": HEIGHT,
        "title": "llm_router — smart model routing for Claude Code",
