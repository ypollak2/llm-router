...
_EXTRA_VERSION_FILES: list[tuple[Path, re.Pattern, str]] = [
    ...
    (ROOT / "src" / "llm_router" / "tui" / "__init__.py", _TUI_VERSION_RE, '__version__ = "{version}"'),
]
...
_ROUTERARENA_VERSION_RE = re.compile(r"(LLM Router router for RouterArena — )v\d+\.\d+\.\d+")
...
def bump_versions(version: str, *, root: Path = ROOT) -> None:
    ...
    routerarena_path = root / "bench" / "routerarena" / "submission" / "router" / "llm_router_router.py"
...
def perform_release(
    ...
    *,
    ...
    if not skip_plugin_reinstall:
        ...
        run(["Codex", "plugin", "remove", "llm_router"], dry_run=dry_run)
        run(["Codex", "plugin", "add", "llm_router"], dry_run=dry_run)
