...
def check_name_consistency(data: dict[str, dict]) -> bool:
    """Check that all plugins are named 'llm_router'."""
    issues = []

    for plugin_dir, files in data.items():
        plugin_name = files["plugin"].get("name")
        if plugin_name != "llm_router":
            issues.append(f"{plugin_dir}/plugin.json: name is '{plugin_name}', expected 'llm_router'")
        ...
        if marketplace_plugins:
            for i, plugin in enumerate(marketplace_plugins):
                if plugin.get("name") != "llm_router":
                    issues.append(
                        f"{plugin_dir}/marketplace.json plugin[{i}]: name is '{plugin.get('name')}', expected 'llm_router'"
                    )
    ...
    print("✅ All plugins named 'llm_router'")
    return True
