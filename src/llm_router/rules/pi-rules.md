...
> These rules apply when using llm_router MCP tools inside Pi (pi.dev coding agent).
...
Pi loads MCP servers from `~/.pi/agent/mcp.json`. Once llm_router is
...
Pi's native model is included in your subscription. llm_router adds value by:
