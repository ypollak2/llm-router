...
def _run_setup() -> None:
    ...
    if new_keys:
        ...
        lines = ["# LLM Router provider keys — auto-generated by `llm_router setup`", ""]
    ...
    print()
    ans = input("Run `llm_router install` now? [Y/n]: ").strip().lower()
    if ans in ("", "y", "yes"):
        from llm_router.commands.install import cmd_install
        cmd_install([])
    else:
        print(f"\n  Run {_bold('llm_router install')} when ready to activate routing.\n")
